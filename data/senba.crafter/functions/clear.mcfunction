...
execute at @a as @e[distance=..9,type=item_frame,tag=!senba.R.crafter,x_rotation=-90,nbt=!{Item:{tag:{}},Fixed:1b}] unless entity @s[nbt=!{Item:{id:"minecraft:paper"}},nbt=!{Item:{id:"minecraft:gold_nugget"}}] at @s positioned ~ ~-1 ~ if block ~ ~ ~ crafting_table run function senba.crafter:loot

#いらないモノを消す
data remove storage senba.clear All
