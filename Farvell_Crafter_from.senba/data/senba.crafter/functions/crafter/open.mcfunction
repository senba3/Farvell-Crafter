...
#レシピを生成したい
execute unless entity @s[tag=senba.crafter-make] if data block ~ ~ ~ {Items:[{id:"minecraft:command_block",Slot:14b}]} unless data block ~ ~ ~ {Items:[{id:"minecraft:command_block",Slot:14b,tag:{}}]} run function senba.crafter:crafter/recipe
execute unless entity @s[tag=senba.crafter-make] if data block ~ ~ ~ {Items:[{id:"minecraft:command_block",Slot:16b}]} unless data block ~ ~ ~ {Items:[{id:"minecraft:command_block",Slot:16b,tag:{}}]} run function senba.crafter:crafter/recipe
...
#アイテムを記憶させる 緊急時に使用
execute unless data block ~ ~ ~ {Items:[{Slot:15b,tag:{senba.crafter-kill_item:1b}}]} run data modify entity @s Item.tag.senba-Items set from block ~ ~ ~ Items

#クラフトレシピがあるかのファンクションを読み込む
tag @s[tag=senba.crafter-recipe] remove senba.crafter-recipe
execute unless entity @e[distance=..2,type=hopper_minecart] if entity @s[tag=!senba.crafter-make_recipe] run function #senba.crafter:recipe
