 ...
 #クラフトファンクション
  ...
  #通常レシピコマンド
   ...
   #クラフト欄にアイテムがあったとき用
    ...
    tellraw @p[tag=senba.crafter-looking] [{"text":"execute as @s[tag="},{"text":"senba.craft+"},{"entity":"@e[type=item,limit=1,tag=senba.crafter-get]","nbt":"UUID[0]"},{"text":"] unless block ~ ~ ~ barrel{Items:["},{"entity":"@e[type=item,limit=1,tag=senba.crafter-get]","nbt":"Item.tag.goods"},{"text":"]}"},{"text":" run tag @s remove "},{"text":"senba.craft+"},{"entity":"@e[type=item,limit=1,tag=senba.crafter-get]","nbt":"UUID[0]"}]
